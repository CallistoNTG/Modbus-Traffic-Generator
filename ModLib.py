...
# Modified by Jacob Soper for SCADA attack dataset framework
#
...
def getSupportedFunctionCodes(c):
    ...
    for i in range(0, 256):  # Total of 127 (legal) function codes
        ans = connection.sr1(ModbusADU(transId=getTransId()) / ModbusPDU_Read_Generic(funcCode=i), timeout=timeout,
                             verbose=0)
        ...
        if ans:
            ...
            data2 = codecs.encode(data.encode(), 'hex_codec')
            test = binascii.hexlify(data.encode())
            # y = bytes(data, "utf-8").decode("unicode-escape")
            # data2 = binascii.hexlify(data.encode('utf8'))
            results = re.findall("[xX][0-9a-fA-F]+", data)

            converted = [int(result[1] + result[2], 16) if result[0] != 0 else int(result[2]) for result in results]

            if(len(converted) < 9):
                returnCode = converted[-2]
                exceptionCode = converted[-1]
            else:
                returnCode = converted[7]
                exceptionCode = converted[8]
...
def getSupportedDiagnostics(c):
    ...
    for i in range(0, 65535):  # Total of 65535, function code 8, sub-function code is 2 bytes long
        ans = connection.sr1(ModbusADU(transId=getTransId()) / Raw("\x08") / struct.pack(">H", i) / Raw("\x00\x00"),
